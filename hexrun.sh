...
outfile=/home/mhn/zephyrproject/applications/blinky/build/zephyr/zephyr.signed.confirmed.encrypted.hex

uf2dir=/media/mhn/XIAO-SENSE

slot_size=0x53000
Slot1_address=0x8d000
...
if [ -f "$infile" ]; then
	$imgtool sign --key $key -E $encrypt --header-size 0x200 -S $slot_size --version $ver --confirm --pad --hex-addr $Slot1_address $infile $outfile
	python3 $uf2  $outfile  -c  -f 0xADA52840
else
	echo -e "${RED}============( Error )=============${NC}"
